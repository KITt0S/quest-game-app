...
public class PlayerRenderer extends Renderer {
    ...
    public void render(RenderContext context) {
        ...
        shader.setUniformVec2(gl, "uPosition", player.getTransform().getPosition().x, player.getTransform().getPosition().y);
        shader.setUniformVec2(gl, "uScale", player.getWidth(), player.getHeight());
        gl.glActiveTexture(GL3.GL_TEXTURE0);
        gl.glTexParameteri(GL3.GL_TEXTURE_2D, GL3.GL_TEXTURE_MIN_FILTER, GL3.GL_LINEAR);
        gl.glTexParameteri(GL3.GL_TEXTURE_2D, GL3.GL_TEXTURE_MAG_FILTER, GL3.GL_LINEAR);
        player.getSprite().getTexture().bind(gl);
